...
> **One-line:** A Docker-friendly **FastAPI** service that scrubs **plain text** and **nested JSON** from a mounted **policy file** (deterministic field rules) plus **best-effort Presidio NLP**, with optional **batch/file ingestion** and **artifact storage**. It centralizes redaction; it is **not** a compliance program, accuracy guarantee, or database firewall (personal / learning project).
...
2. **Realtime** — `POST /v1/sanitize` with `X-API-Key`; free text + nested `structured` JSON through the same pipeline (per-field rules first; best-effort NLP on undeclared strings).
3. **Batch / files** — `/internal/jobs/*` (or scheduler) for named Postgres queries and inbox scans; prefer declared field rules over NLP alone for tabular exports.
...
- **English-only** Presidio language path; NLP false positives/negatives are normal—public demo copy says so
